#!/usr/bin/env bash
# lowakernel-universal build script
...
apply_config() {
  ...
  if [[ "$running_kernel" != *"-zen"* || "$running_kernel" == *"lowakernel"* ]]; then
    error "You're running ${running_kernel}. Boot into vanilla linux-zen first, the base config must come from a stock zen kernel, not from linux/lts or an already-stripped lowakernel."
  fi
  ...
  # 300hz - same as vanilla zen, good balance for a laptop
...
install_kernel() {
  ...
  if find "/lib/modules/${kver}" -name "amdgpu.ko*" -print -quit 2>/dev/null | grep -q .; then
    success "amdgpu: module (=m), no firmware FILES needed"
  else
    warn "amdgpu is built-in (=y), appending firmware FILES to ${own_conf}"
    local fw_list
    fw_list=$(find /lib/firmware/amdgpu/ \( -name "*.bin" -o -name "*.bin.zst" \) \
              2>/dev/null | sort | sed 's/^/  /')
    # goes into own_conf only, other kernels' initramfs stay untouched
    printf '\n# lowakernel: amdgpu firmware for built-in driver\nFILES=(\n%s\n)\n' "$fw_list" | sudo tee -a "$own_conf" > /dev/null
    warn "Appended firmware FILES (temporary, fix DRM_AMDGPU=m and rebuild)"
  fi
...
EOF
  ...
  # grub/refind/limine folks need the modules separately since they dont get
  # the uki. ask if we should pack them, so systemd-boot people dont get a
  # random 50mb tarball they never asked for lol
  echo ""
  read -rp "  Pack modules archive for a release? [y/N] " pack_ans
  if [[ "${pack_ans,,}" == "y" ]]; then
    local mods_archive="$HOME/Downloads/modules-${kver}.tar.zst"
    info "Packing modules, this takes a sec..."
    mkdir -p "$HOME/Downloads"
    sudo tar -C /lib/modules -caf "$mods_archive" "${kver}"
    sudo chown "$(id -un):$(id -gn)" "$mods_archive"
    success "Modules archive ready: ${mods_archive}"
  fi
}
...
main() {
  ...
  echo -e "║  P15v G1-G3 · L14/L15 G1-G4 · E14 G6               ║"
  echo -e "╚════════════════════════════════════════════════════╝${NC}\n"
