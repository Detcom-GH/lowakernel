...
Download the modules archive from the release and extract it (required, because initramfs generation will fail without it):
 
```bash
sudo tar -C /lib/modules -xaf modules-lowakernel-universal.tar.zst
```
 
Generate initramfs for your distro (replace `7.2.2` with the kernel version of the release you downloaded):
 
```bash
# Arch / Arch-based
sudo mkinitcpio -k 7.2.2-lowakernel-universal -g /boot/initramfs-lowakernel-universal.img
 
# Fedora
sudo dracut /boot/initramfs-lowakernel-universal.img 7.2.2-lowakernel-universal
 
# Ubuntu / Debian
sudo update-initramfs -c -k 7.2.2-lowakernel-universal
```
...
Extract the modules archive and generate initramfs (same commands as the GRUB section above), then add a stanza to `/boot/refind_linux.conf`:
...
Extract the modules archive and generate initramfs (same commands as the GRUB section above), then add an entry to your `limine.conf`:
...
For the UKI route, copy the `.efi` file to your ESP first, the UKI carries its own cmdline, initramfs and modules, so the modules archive is not needed.
...
sudo rm -rf /lib/modules/7.2.2-lowakernel-universal
```
 
(adjust the version to match what you installed)
